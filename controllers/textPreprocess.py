...
LOG_LEVELS = ['info', 'debug', 'trace', 'warning', 'fatal'] # 5 more dimensions for the vectors
...
def compute_log_vector(log: dict) -> np.ndarray:
    ...
    - One-hot vector for logLevel - for example: [0,0,0,1,0] if warning is in the log
    ...
    words = log_string.lower().split() # the array of each word
    if words:
