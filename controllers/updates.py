...
# Load trained model
fullyConnectedLayer = FullyConnectedLayer(input_dim=VECTOR_SIZE, output_dim=2)
fullyConnectedLayer.load_model()
...
def update_biases(real_answer, predicted_class):
    ...
    fullyConnectedLayer.biases[predicted_class] += adjustment
...
def update_word_vectors(log, real_answer, predicted_class):
    """
...
def update_weights(fullyConnectedLayer, full_vector, y_true, learningRate=0.5):
    """Updates classifier weights (W) and biases (b) using gradient descent."""
    z = fullyConnectedLayer.forward(full_vector)
    probabilities = softmax(z)
    error = y_true - probabilities

    # print(f" BEFORE UPDATE: Mean(W)={np.mean(fullyConnectedLayer.weights)}, Mean(b)={np.mean(fullyConnectedLayer.biases)}")
    # print(f" ERROR VECTOR: {error}")

    weight_update = learningRate * np.outer(error, full_vector)
    bias_update = learningRate * error

    fullyConnectedLayer.weights += weight_update
    fullyConnectedLayer.biases += bias_update

    # print(f" AFTER UPDATE: Mean(W)={np.mean(fullyConnectedLayer.weights)}, Mean(b)={np.mean(fullyConnectedLayer.biases)}")
